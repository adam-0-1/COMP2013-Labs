...
//WRITE YOUR CODE BELOW
console.log(
  `This house at ${listing_1.address} is owned by ${listing_1.current_owner}`
);
...
function realtorFees(listing) {
  ...
  const fee_rate = price <= 450_000 ? 0.025 : 0.02;
  return (price * fee_rate).toString();
}
/**
 * Task-5: Sort the listing array ascendingly in a new variable called listingAscendingly
 */
//WRITE YOUR CODE BELOW
const listing_ascendingly = [...listings].sort((a, b) => {
  const price_a = Number(a.price.replace(/[$,]/g, ""));
  const price_b = Number(b.price.replace(/[$,]/g, ""));
 ...
 */
//WRITE YOUR CODE BELOW
const new_built_listings = listings.filter(listing => {
  const year_str = listing.propertySummary.builtIn;
